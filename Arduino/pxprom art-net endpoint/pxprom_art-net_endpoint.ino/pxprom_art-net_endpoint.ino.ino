...
// OctoWS2811 setup
const int ledsPerStrip = 115; // only using 2 data pins, but zig-zagging data over 4 rows each (configured in MADRIX)
...
const int maxUniverses = numberOfChannels / 512 + ((numberOfChannels % 512) ? 1 : 0); // often 1
...
void bufferSensorReads() {
  ...
  // Read from sensors
  unsigned int newRead_a = sonar_a.ping_cm();
  unsigned int newRead_b = sonar_b.ping_cm();
  Serial.print("A: ");
  Serial.println(newRead_a);
  Serial.print("B: ");
  Serial.println(newRead_b);

  // Place readings in ArtNet Packet
  if (BETWEEN(newRead_a, prevRead_a - changeBuffer, prevRead_a + changeBuffer)) {
    DmxBuffer[0] = prevRead_a;
  }
  else {
    DmxBuffer[0] = newRead_a;
  }

  if (BETWEEN(newRead_b, prevRead_b - changeBuffer, prevRead_b + changeBuffer)) {
    DmxBuffer[1] = prevRead_b;
  }
  else {
    DmxBuffer[1] = newRead_b;
  }

  prevRead_a = newRead_a;
  prevRead_b = newRead_b;
